...
describe('createAdcpServer', () => {
  ...
  describe('protocol task tools', () => {
    ...
    it('registers protocol task tools for the release-precision 3.1-rc.13 pin', async () => {
      const taskRegistry = createInMemoryTaskRegistry();
      const server = createAdcpServer({
        name: 'Test',
        version: '1.0.0',
        adcpVersion: '3.1-rc.13',
        taskRegistry,
      });
      const tools = registeredTools(server);
      assert.ok(tools.includes('get_task_status'));
    ...
    it('answers get_task_status/list_tasks from the scoped AdCP task registry only', async () => {
      ...
      assert.strictEqual(status.adcp_version, '3.1-rc.13');
      ...
      assert.strictEqual(listed.adcp_version, '3.1-rc.13');
      ...
      assert.strictEqual(badCursor.structuredContent.adcp_version, '3.1-rc.13');
    ...
    it('applies authInfo credential scanning to protocol task aliases', async () => {
      ...
      assert.strictEqual(status.structuredContent.adcp_version, '3.1-rc.13');
      ...
      assert.strictEqual(listed.structuredContent.adcp_version, '3.1-rc.13');
