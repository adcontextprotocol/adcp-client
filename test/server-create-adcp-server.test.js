...
describe('createAdcpServer', () => {
  ...
  it('separates the unversioned default from the supported ceiling across dispatch and discovery', async () => {
    ...
    assert.strictEqual(server.getAdcpVersion(), '3.2.0-rc.6');
    assert.strictEqual(server.getDefaultAdcpVersion(), '3.1.18');

    const defaultProducts = await callTool(server, 'get_products', {});
    const modernProducts = await callTool(server, 'get_products', { adcp_version: '3.2.0-rc.6' });
    assert.strictEqual(defaultProducts.adcp_version, '3.1');
    assert.strictEqual(modernProducts.adcp_version, '3.2-rc.6');
    ...
    await callTool(server, 'version_probe', { adcp_version: '3.2.0-rc.6' });
    assert.deepStrictEqual(
    ...
    assert.strictEqual(modernCapabilities.adcp_version, '3.2-rc.6');
    ...
    const modernTools = await server.dispatchTestRequest({
      method: 'tools/list',
      params: { _meta: { adcp_version: '3.2.0-rc.6' } },
    });
    ...
    assert.strictEqual(modernTools._meta.adcp_version, '3.2.0-rc.6');
    ...
    assert.throws(
      () =>
        createAdcpServer({
          ...
          adcpVersion: '3.2.0-rc.6',
          defaultAdcpVersion: '3.1.18',
          capabilities: { supported_versions: ['3.2.0-rc.6'] },
        }),
  ...
  describe('MCP media-buy tool profile', () => {
    ...
    it('advertises the active 3.2 profile by default while legacy aliases remain callable', async () => {
      ...
      const server = createAdcpServer({
        ...
        version: '1.0.0',
        adcpVersion: '3.2.0-rc.6',
        capabilities: { supported_versions: ['3.0.25', '3.1.18', '3.2.0-rc.6'] },
        mediaBuy: {
          ...compactHandlers,
          getProducts: async params => {
      ...
      assert.strictEqual(listed.tools[0]._meta.adcp_version, '3.2.0-rc.6');
  ...
  describe('auto-generated capabilities', () => {
    ...
    it('advertises exactly the registered compact lifecycle tools and strips them on legacy pins', async () => {
      ...
      const modern = await callTool(server, 'get_adcp_capabilities', { adcp_version: '3.2-rc.6' });
  ...
  describe('protocol task tools', () => {
    ...
    it('answers get_task_status/list_tasks from the scoped AdCP task registry only', async () => {
      ...
      assert.strictEqual(status.adcp_version, '3.2-rc.6');
      ...
      assert.strictEqual(listed.adcp_version, '3.2-rc.6');
      ...
      assert.strictEqual(badCursor.structuredContent.adcp_version, '3.2-rc.6');
    ...
    it('applies authInfo credential scanning to protocol task aliases', async () => {
      ...
      assert.strictEqual(status.structuredContent.adcp_version, '3.2-rc.6');
      ...
      assert.strictEqual(listed.structuredContent.adcp_version, '3.2-rc.6');
