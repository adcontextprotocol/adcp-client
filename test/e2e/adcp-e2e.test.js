...
class AdCPE2ETest {
  ...
  async testParameterVariations() {
    ...
    await this.test('Missing Tool Parameter Error', async () => {
      ...
      assert(result.data.error.includes('Missing required parameter'), 'Error message should mention missing parameter');
    });
  }
  ...
  async testErrorHandling() {
    await this.test('Invalid Agent ID', async () => {
      ...
      assert(result.status === 404 || (result.status === 200 && result.data.success === false), 
    ...
    await this.test('Invalid Tool Name', async () => {
      ...
      assert(result.status >= 400 || (result.status === 200 && result.data.success === false), 
