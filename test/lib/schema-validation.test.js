...
describe('schema-driven validation', () => {
  ...
  describe('validateResponse', () => {
    ...
    test('accepts SDK-stamped adcp_version through nested compact commitment aliases', () => {
      for (const tool of ['buy_products', 'accept_proposal']) {
        for (const response of [
          compactCommitmentResponse({ adcp_version: '3.2-rc.6' }),
          failedCompactCommitmentResponse({ adcp_version: '3.2-rc.6' }),
        ]) {
          const outcome = validateResponse(tool, response, '3.2.0-rc.6');
    ...
    test('compact commitment relaxation does not make nested bodies permissive', () => {
      ...
      const outcome = validateResponse('buy_products', response, '3.2.0-rc.6');
      assert.ok(
        outcome.issues.some(
          issue =>
