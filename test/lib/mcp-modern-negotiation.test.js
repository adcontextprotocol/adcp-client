...
test('modern serving honors the resolved AdCP MCP tool profile', async () => {
  ...
  assert.equal(compact._meta.adcp_version, '3.2.0-beta.4');
  ...
  assert.equal(all._meta.adcp_version, '3.2.0-beta.4');
...
test('modern serving preserves explicitly registered custom schemas and descriptions', async t => {
  ...
  const adapter = createModernMcpServerAdapter(wrapMcpServer(legacy, undefined, '3.2.0-beta.4'));
