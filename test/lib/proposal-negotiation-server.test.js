...
describe('refine_proposals server integration', () => {
  ...
  test('modern platform seam auto-pins to an advertised 3.2 release', async () => {
    const server = createAdcpServerFromPlatform(
      {
        capabilities: { specialisms: [], supported_versions: ['3.2-rc.1'] },
        accounts: {
          resolution: 'derived',
    ...
    assert.equal(capabilities.structuredContent.adcp_version, '3.2-rc.1');
    assert.deepEqual(capabilities.structuredContent.adcp.supported_versions, ['3.2-rc.1']);
  ...
  test('registers as a framework tool and projects proposal capabilities', async () => {
    ...
    assert.equal(capabilities.structuredContent.adcp_version, '3.2-rc.2');
    ...
    assert.equal(response.structuredContent.adcp_version, '3.2-rc.2');
  });
  ...
  test('hides proposal refinement capabilities from a negotiated pre-3.2 response', async () => {
    const server = negotiationServer(undefined, {
      serverCapabilities: { supported_versions: ['3.1', '3.2-rc.1'] },
    });
    ...
    assert.equal(modern.structuredContent.adcp_version, '3.2-rc.1');
