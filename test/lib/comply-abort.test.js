...
async function withComplianceDir(complianceDir, fn) {
  const previous = process.env.ADCP_COMPLIANCE_DIR;
  process.env.ADCP_COMPLIANCE_DIR = complianceDir;
  try {
    return await fn();
  } finally {
    if (previous === undefined) {
      delete process.env.ADCP_COMPLIANCE_DIR;
    } else {
      process.env.ADCP_COMPLIANCE_DIR = previous;
    }
  }
}
...
describe('comply() timeout_ms option', () => {
  test('timeout_ms stops new storyboards without aborting the active storyboard', async () => {
    ...
    try {
      const result = await withComplianceDir(complianceDir, () =>
        comply(agent.url, {
          allow_http: true,
...
describe('comply() combined timeout_ms + signal', () => {
  ...
  test('signal aborts an active storyboard step', async () => {
    ...
    try {
      await withComplianceDir(complianceDir, () =>
        assert.rejects(
          () =>
            comply(agent.url, {
              allow_http: true,
