...
- **`idempotency_key`** — required on every mutating request. Applies to every tool marked as mutating in the spec (the SDK's `MUTATING_TASKS` constant is authoritative — 28 tools): `create_media_buy`, `update_media_buy`, `sync_accounts`, `sync_creatives`, `build_creative`, `sync_audiences`, `sync_catalogs`, `sync_event_sources`, `sync_plans`, `sync_governance`, `provide_performance_feedback`, `acquire_rights`, `update_rights`, `activate_signal`, `log_event`, `report_usage`, `report_plan_outcome`, `create_property_list` / `update_property_list` / `delete_property_list`, `create_collection_list` / `update_collection_list` / `delete_collection_list`, `create_content_standards` / `update_content_standards` / `calibrate_content`, `si_initiate_session` / `si_send_message`. Full skill-domain breakdown in [`skills/cross-cutting.md`](./skills/cross-cutting.md). Handlers must return the same response when the same key is replayed. Landed in AdCP 3.0 GA.
