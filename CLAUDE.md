...
**Migrating from 6.6 to 6.7?** See [`docs/migration-6.6-to-6.7.md`](./docs/migration-6.6-to-6.7.md) — twelve adopter recipes including the two breaking changes adopters must audit before bumping (#10: `accounts.resolution: 'implicit'` now actually refuses inline `{account_id}` references — was aspirational pre-6.7; #11: `SalesPlatform` split into `SalesCorePlatform & SalesIngestionPlatform` with all methods individually optional, so `: SalesPlatform<Meta>` annotations claiming sales-non-guaranteed / -guaranteed / -broadcast-tv / -catalog-driven need to migrate to `: SalesCorePlatform<Meta> & SalesIngestionPlatform<Meta>` or use the new `defineSalesCorePlatform` + `defineSalesIngestionPlatform` spread helpers). Mechanical recipes cover `definePlatform` / `defineSalesCorePlatform` / `defineSalesIngestionPlatform` (drop `req: unknown` casts), `refAccountId(ref)`, `composeMethod`, `accounts.resolve` security presets (`requireAccountMatch` / `requireAdvertiserMatch` / `requireOrgScope` — composable post-resolve guards with default-`null` deny to avoid principal enumeration), `accounts.upsert(refs, ctx)` / `accounts.list(filter, ctx)` principal-keyed gating, typed errors (`PermissionDeniedError` / `AuthRequiredError` plus the auth/availability/governance families), `issues[].hint` + oneOf near-miss diagnostics, `BuyerAgentRegistry`, `createTenantRegistry`, and `createOAuthPassthroughResolver` for Shape B (vendor OAuth + `/me/adaccounts` upstream). Companion: `NoAccountCtx<TCtxMeta>` narrow for `previewCreative` / `listCreativeFormats` / `providePerformanceFeedback` adopters who dereferenced `ctx.account` unconditionally. Plus `instructions` function form, `listCreativeFormats?` on creative platforms, `update_rights` first-class, spec-driven auto-hydration via `x-entity` annotations, `runStoryboard({ agents })` per-specialism storyboard routing, `InMemoryImplicitAccountStore` reference adapter, and the worked-reference `examples/hello_*_adapter_*` family.
