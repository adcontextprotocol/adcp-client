...
- 13/13 smoke tests pass: build server from platform, dispatch all 5 sales tools (sync + submitted + rejected projection), creative + audience tools dispatch, ACCOUNT_NOT_FOUND envelope on AccountNotFoundError throw, validatePlatform forward-compat.

Status: Preview / 6.0. The runtime is not yet exported from `./server`; reach in via `@adcp/client/server/decisioning/runtime` for spike experimentation only. Subsequent commits land `taskHandle.notify` integration with the framework's task store + webhook emitter, `partialResult` projection, per-tenant `getCapabilitiesFor` runtime, and "framework always calls `accounts.resolve(authPrincipal)`" behavior for `'derived'` / `'implicit'` resolution modes. Companion design doc at `docs/proposals/mcp-a2a-unified-serving.md`.
