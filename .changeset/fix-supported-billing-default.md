...
**Type widening (folded from #1199):** `DecisioningCapabilities.supportedBillings` is widened from `'operator' | 'agent'` to `'operator' | 'agent' | 'advertiser'`. The AdCP schema enum (`schemas/cache/3.0.1/enums/billing-party.json`) has always included `'advertiser'` (seller invoices the advertiser directly — Google Ads direct, Meta direct, retail-media-adjacent platforms); the TypeScript type was incorrectly narrower. Additive widening — no breaking change for existing adopters.

Related upstream issue: adcontextprotocol/adcp#3746 (make `supported_billing` conditional on `supported_protocols.includes('media_buy')` at the schema level — the principled fix). This SDK change is the defensive default until the spec change lands. Closes adcontextprotocol/adcp-client#1199.
