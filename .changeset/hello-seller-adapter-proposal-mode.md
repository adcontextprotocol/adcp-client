...
Companion CI gate: `test/examples/hello-seller-adapter-proposal-mode.test.js` registers the standard three-gate run (strict tsc, storyboard pass, façade upstream-traffic check). Storyboard target: `media_buy_seller/proposal_finalize` — the spec's canonical brief→finalize→accept scenario.
