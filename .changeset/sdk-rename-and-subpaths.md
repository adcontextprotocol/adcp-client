...
Repo restructure: top-level `package.json` now declares an npm workspace covering `.` plus `packages/*`. The two packages stay version-linked via `.changeset/config.json` so they always release at the same number; the shim's `peerDependencies."@adcp/sdk"` covers the published range (`^5.21.1`) so npm 7+ auto-installs and dedupes consumers' trees rather than producing two physical copies.
