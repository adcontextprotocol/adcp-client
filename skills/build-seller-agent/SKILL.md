...
| ---------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `sales-guaranteed`     | stable  | `create_media_buy` has **three return shapes**, routed on request signals: (a) IO-signing required → task envelope (`status: 'submitted'`, `task_id`, no `media_buy_id` yet); (b) packages without `creative_assignments` → synchronous MediaBuy with `status: 'pending_creatives'` + `media_buy_id`; (c) instant-confirmable → synchronous MediaBuy with `status: 'active'` + `media_buy_id`. Aggressive `measurement_terms` → `adcpError('TERMS_REJECTED')`. **Read the companion file before writing handlers** — applying only the task-envelope path fails 5 storyboard create_media_buy steps that probe the synchronous branches.                                                                                                                                                                                                                                              | [§ sales-guaranteed](#specialism-sales-guaranteed)         |
