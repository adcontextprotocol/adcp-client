...
Audio templates (TTS / mix / master, e.g. AudioStack / ElevenLabs / Resemble) follow the same shape with two deltas: (a) format declares `parameterizedRender({ role: 'primary' })` because audio has no width/height — encode duration/codec/bitrate in `accepts_parameters`, not in render entries; (b) the upstream pipeline is naturally minutes-long (voice-over → mix → master), so the adapter returns a task envelope and emits `creative_review` webhooks on completion. Output projects via `audioAsset({ url })` in the creative-manifest (the framework injects the `asset_type: 'audio'` discriminator).

The mock-server's `creative-template` specialism ships an audio template (`tpl_audiostack_spot_30s_v1`) and the `synthesizeOutput` returns `{ audio_url, preview_url, assets: [{ kind: 'audio_url', mime_type: 'audio/mpeg' }] }` — the canonical `hello_creative_adapter_template.ts` projects it. Audio adopters fork the same display adapter and add an `audio_url` branch to their `projectRenderToManifest`. *Storyboard coverage for audio* is not yet upstream — tracked at adcontextprotocol/adcp (the storyboard `creative_template/build_creative` step asserts display-shaped assets); audio adopters validate via `npm run compliance:fork-matrix -- --test-name-pattern="hello-creative-adapter-template"` (display + video gate) plus a manual round-trip against the seeded audio template.
