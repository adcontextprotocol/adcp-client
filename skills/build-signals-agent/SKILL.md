...
> - **`activation_key` layout for `key_value` type** — `key` and `value` are **top-level** fields, not nested under a `key_value` sub-object:
>   ```ts
>   // ✗ WRONG — extra nesting causes oneOf validation failure:
>   { type: 'key_value', key_value: { key: 'audience', value: 'abc' } }
>   // ✓ RIGHT — flat, top-level:
>   { type: 'key_value', key: 'audience', value: 'abc' }
>   ```
> - **`signal_ids` lookup element shape** (in `get_signals` requests with `signal_ids` mode) — each element is a single `SignalID` discriminated-union object, not a flat string or nested sub-array:
>   ```ts
>   // ✗ WRONG — `id` field is a string, not an array:
>   { source: 'catalog', data_provider_domain: 'provider.com', ids: ['seg_abc'] }
>   // ✓ RIGHT — each element (marketplace):
>   { source: 'catalog', data_provider_domain: 'provider.com', id: 'seg_abc' }
>   // ✓ RIGHT — each element (owned):
>   { source: 'agent', agent_url: 'https://signals.example.com/mcp', id: 'seg_xyz' }
>   ```
