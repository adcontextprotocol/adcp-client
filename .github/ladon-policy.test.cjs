...
const notice = steps.find(step => step.name === 'Comment and skip when PR modifies review workflow');
...
function collectInvocations(value, location, result = []) {
  ...
  if (typeof value.uses === 'string' && /adcontextprotocol\/actions\/ladon\//i.test(value.uses)) {
...
test('all tracked workflow and local-action Ladon sites are inventoried, pinned, and disabled', () => {
  const files = git('ls-files', '-z', '--', '.github/workflows', ':(glob)**/action.yml', ':(glob)**/action.yaml')
    .split('\0')
    .filter(file => /^\.github\/workflows\/[^/]+\.ya?ml$/.test(file) || /(^|\/)action\.ya?ml$/.test(file));
  const sites = files.flatMap(file => collectInvocations(parse(read(file)), file));
  assert.equal(sites.length, 1, 'new invocation sites require an explicit human-reviewed policy update');
  assert.match(sites[0].location, /^\.github\/workflows\/ai-review\.yml\.jobs\.code_review\.steps\./);
...
test('consumer contract rejects missing, malformed, enabled, boolean, and expression inputs', () => {
  ...
  for (const pin of ['ladon/review/v1', 'main', IMPLEMENTATION]) {
    assert.throws(() => validateInvocation({ ...invocation, uses: `adcontextprotocol/actions/ladon/review@${pin}` }));
  }
});
...
test('trusted base checkout, draft guard, modification gate, and COMMENT-only human notice remain', async () => {
  ...
  assert.deepEqual(workflow.on.pull_request_target['paths-ignore'], ['.github/workflows/ai-review.yml', 'LADON.md']);
  assert.match(workflow.jobs.code_review.if, /github\.event\.pull_request\.draft == false/);
  ...
  await new AsyncFunction('github', 'context', 'process', notice.with.script)(
    ...
    { repo: { owner: 'adcontextprotocol', repo: 'test' }, issue: { number: 1 } },
...
test('human hold and two distinct protection bypasses remain visible', () => {
  ...
  for (const marker of [
    ...
    assert.ok(adoption.includes(marker), `missing hold/audit evidence: ${marker}`);
  }
});
