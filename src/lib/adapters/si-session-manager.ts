...
export class SISessionManager implements ISISessionManager {
  ...
  async initiateSession(request: SIInitiateSessionRequest): Promise<SIInitiateSessionResponse> {
    ...
    const session: SISession = {
      ...
      mediaBuyId: request.media_buy_id ?? undefined,
      offeringId: request.offering_id ?? undefined,
      offeringToken: request.offering_token ?? undefined,
      placement: request.placement ?? undefined,
      negotiatedCapabilities: this.negotiateCapabilities(request.supported_capabilities ?? undefined),
      status: 'active',
      messageCount: 0,
      conversationHistory: [],
    };
