...
export const STANDARD_ERROR_CODES_FROM_MANIFEST = {
  ...
  "COMMITTED_RESOURCE_PURGED": {
    description: "The requested mutation committed, but the affected resource was independently deleted or purged for a reason outside that requested mutation before the seller could durably record the canonical success response. The write stands even though the resource no longer exists; this code does not describe the successful outcome of a requested delete operation. Sellers MUST retain the idempotency key record and canonical request hash independently of the resource until at least committed_at + replay_ttl_seconds, and exact replays MUST return this terminal outcome with replayed: true without re-executing the mutation. If the canonical success response was already durably recorded, sellers return that historical response under the normal replay rule instead. Distinct from CONFLICT (a transient concurrent modification before a stable result) and MEDIA_BUY_NOT_FOUND (a request targeting a resource that was already absent).",
    recovery: "terminal",
    suggestion: "do not automatically retry with the same or a fresh idempotency_key; reconcile through the resource read task and seller audit or operator records before deciding whether to initiate a new operation"
  },
  ...
  "CONFLICT": {
    description: "Concurrent modification detected. The resource was modified by another request between read and write. This code does not cover a resource that was committed and then independently deleted or purged before the seller could durably record the canonical success response; use COMMITTED_RESOURCE_PURGED for that terminal outcome.",
    recovery: "transient",
  ...
  "CREATIVE_REPRESENTATION_UNRESOLVED": {
    description: "No representation in a CreativeRepresentationSet is compatible with the selected target capability or product format option. The resolver MUST retain the complete representation set and MUST include one `error.details.representation_rejections[]` entry per candidate, conforming to `error-details/creative-representation-unresolved.json`; silent seller guessing is forbidden.",
    recovery: "correctable",
    suggestion: "inspect every error.details.representation_rejections entry, then choose a compatible format option or supply a compatible representation"
  },
  ...
  "IDEMPOTENCY_IN_FLIGHT": {
    description: "A prior request with the same `idempotency_key` is either actively executing or has an unresolved outcome under fail-closed reconciliation, and has not yet produced a cached response or committed-outcome tombstone. Sellers MAY return this code instead of blocking the second caller while active execution or reconciliation finishes — useful when the first call invokes a slow or ambiguously responding downstream system (SSP, ad server, payment provider). Distinct from IDEMPOTENCY_CONFLICT (different canonical payload — a client bug) and from CONFLICT (concurrent modification of a different resource) — IDEMPOTENCY_IN_FLIGHT tells the buyer that its retry is correct but the prior attempt's outcome is not yet available. Sellers SHOULD populate top-level `error.retry_after` with an integer-second wait hint based on expected execution or reconciliation progress. Buyers MUST treat this as transient and MUST NOT mint a fresh `idempotency_key` — minting a new key turns a safe retry into a double-execution race.",
    recovery: "transient",
  ...
  "REQUOTE_REQUIRED": {
    description: "A control_media_buy request, or the 3.x update_media_buy facade, would exceed the accepted commercial envelope. The seller is declining the requested shape at the current terms. Distinct from TERMS_REJECTED (measurement) and POLICY_VIOLATION (content). Error details SHOULD conform to error-details/requote-required.json: populate envelope_field with the field path(s) that breached the envelope, change_term_id when an accepted change term governed the request, and decline_reason when a structured seller-policy reason is available and safe to disclose. All producer-authored explanatory text, including message, suggestion, and extension detail values, MUST remain buyer-safe and MUST NOT contain vendor identifiers, internal object names, internal IDs, stack traces, or other producer-private implementation details. AdCP 3.2 callers refine the MediaBuy's accepted_proposal_id to obtain a typed amendment; legacy 3.1 callers adjust the update, rediscover terms, or create a separate buy.",
    recovery: "correctable",
    suggestion: "use error.details.decline_reason when present to inform recovery, then refine accepted_proposal_id and apply the amendment through accept_proposal, or keep control_media_buy inside the accepted envelope; some policies require a free-text ask or human negotiation, and 3.1 compatibility callers must adjust or rediscover"
  },
  "SCOPE_INSUFFICIENT": {
