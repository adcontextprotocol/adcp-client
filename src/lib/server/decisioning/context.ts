...
export interface RequestContext<TAccount extends Account = Account> {
  ...
  /**
   * Start a framework-managed async task. Returns a `TaskHandle` whose
   * `taskId` is generated by the framework and whose `notify(...)` is
   * pre-wired to the framework's task registry — adopters call
   * `notify(update)` from any context (webhook handler, cron, in-process
   * worker) and the framework persists the lifecycle and pushes to buyer
   * webhooks.
   *
   * Use when returning `submitted(handle, ...)`. The framework owns the
   * taskId namespace and the notify dispatch path.
   *
   * Adopters whose async backend already produces stable task IDs can
   * still construct a `TaskHandle` themselves with their own `taskId`
   * and `notify` — the framework treats `submitted({ taskHandle })` as
   * platform-supplied either way. `ctx.startTask()` is the canonical
   * convenience.
   */
