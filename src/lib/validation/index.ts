...
export function validateAgentUrl(url: string): void {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      if (['localhost', '127.0.0.1', '0.0.0.0', '::1'].includes(hostname) || hostname.startsWith('192.168.') || hostname.startsWith('10.') || hostname.match(/^172\.(1[6-9]|2[0-9]|3[01])\./)) {
 ...
 */
export async function handleAdCPResponse(response: Response, expectedSchema: string, agentName: string): Promise<{ success: boolean; data?: any; error?: string; warnings?: string[] }> {
