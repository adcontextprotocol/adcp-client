...
export interface TransportOptions {
   ...
   *
   * @remarks
   * **Safe for long-lived sessions.** `text/event-stream` responses
   * (the MCP transport's server-initiated-message side channel) are
   * automatically excluded from the cap so cumulative SSE frame bytes
   * never trip it. The limit applies only to buffered (non-streaming)
   * HTTP responses. The cap is scoped per individual network response
   * rather than the full call span, so retried calls each get a fresh
   * counter and long-lived MCP sessions coexist with capped one-shots
   * in the same process.
   *
   * @remarks
...
export class ProtocolClient {
  ...
  static async callTool(
    ...
    // Helper: enter the size-limit ALS slot for one network response. Called
    // at each protocol call site rather than around the outer span so the
    // counter resets between retries and long-lived MCP sessions coexist
    // with capped one-shots in the same process.
    const withSizeLimit = <T>(fn: () => Promise<T>): Promise<T> =>
      withResponseSizeLimit(transport?.maxResponseBytes, fn);
    return withSpan(
