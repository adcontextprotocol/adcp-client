...
try {
  ...
  // CRITICAL: If A2A client fails to import, the application should fail
  console.error('❌ CRITICAL: A2A SDK import failed:', error instanceof Error ? error.message : String(error));
  throw new Error('A2A SDK is required. Please install @a2a-js/sdk');
}
import { 
  AgentConfig, 
  TestResult, 
...
/**
 * Test MCP agent using official MCP client
 */
async function testMCPAgent(
  ...
  try {
    ...
    const result = await circuitBreaker.call(async () => {
      ...
      // Create SSE transport for MCP (MCP uses SSE over HTTP)
