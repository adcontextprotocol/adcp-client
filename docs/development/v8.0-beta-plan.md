...
Current plan ([v3.1-sdk-design.md](./v3.1-sdk-design.md), 396 lines): ship 3.1 capabilities additively across 7.10 → 7.12+ minors, keeping the GA pin on the latest 3.0.x line, with the V2-only break deferred to whenever the spec's v1 deprecation calendar tightens (2027-Q4 floor / 2029-Q1 ceiling).
...
2. **Flip the GA pin** in `src/lib/version.ts` and codegen config from the latest 3.0.x patch to `3.1.0-beta.1`. Regenerate types. Fix the resulting compile errors — this is the **first useful audit** of what 3.1 actually changes in the typed surface.
...
|---|---|---|---|
| `@adcp/client@latest` | 7.x | 3.0.18 | Maintenance-mode |
...
- Bug fixes against the 3.0.x-pinned code.
- Security advisories.
