...
For adapter-specific gates that can reuse the already-booted example and mock server, pass `extraMcpAssertions` to `runHelloAdapterGates({...})`. When the invariant is about the already-run storyboard report itself, use an `extraStoryboards[].assertResult` post-grader assertion instead of dispatching duplicate MCP calls. Use separate inline `it()` blocks only when the gate needs a different process topology, a different server lifecycle, or non-MCP setup. Don't extend the helper beyond generic lifecycle plumbing; some duplication is correct for truly bespoke gates.
