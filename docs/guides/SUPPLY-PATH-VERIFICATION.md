...
Authoritative locations are pinned on first observation with `InMemorySupplyPathAuthorityStore`. A later pointer change rejects before fetching the new location. Configure a durable shared `SupplyPathAuthorityStore` across restarts or workers; `check` must atomically pin a new publisher and reject a changed location. After independently confirming a publisher migration, update the durable pin or call `approveChange` on your in-memory instance. For the process default, import `defaultSupplyPathAuthorities` from `@adcp/sdk` and call `defaultSupplyPathAuthorities.approveChange(publisherDomain, confirmedHttpsLocation)`. Never approve changes from an automatic retry. First-observation pinning cannot detect an origin already compromised before the first observation.
