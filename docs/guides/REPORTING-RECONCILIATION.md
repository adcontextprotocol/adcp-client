...
| 2 | `ExpectedReportingPeriod.officialAfterSeconds` when `requiredFinality` is `official`, falling back to `deliverySlaSeconds`; `deliverySlaSeconds` otherwise | Added to the period end. `reporting-schedule.json` defines only `delivery_sla`, with no finality qualifier — `official_after` is an SDK-local extension — so `deliverySlaSeconds` is the spec-defined answer for a seller that does not carry it. **Record `officialAfterSeconds >= deliverySlaSeconds`.** A shorter pin makes the period go overdue earlier than the seller's own schedule would. For `received` and `content_mismatch` that is harmless — `status_as_of` comes from your read clock — but `obligation_missing` and `revision_missing` are dated from the derived `expected_at`, so if the seller recomputes a later one its `expected_period` check refuses them, identically on every run. Those two show up in `failedConsumerStatuses`, not as silence. |
...
Three more conditions deserve an alert, because none of them sets `suppressed`:
...
`result.escalations` projects the seller's own `obligation.issues` into work items, with the operations contact attached. It is capped at 256 per obligation — seller-supplied and counted against no ledger limit — so treat it as a paging signal rather than a complete inventory, and read `obligation.issues` directly if you need every one.
