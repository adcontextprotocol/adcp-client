...
| `period_identity_unknown` | `period.source_timezone` is part of the chain's logical key and could not be established. Three causes: the seller's value is not a recognized IANA zone; your own `periodSourceTimezone` pin is not; or both are valid **and they name different zones**. | `reason` names which, and quotes the values. Fix the pin, or take the seller's value up with the seller. Nothing is posted in the disagreement case on purpose: the seller compares this field by string identity, so your pin would be refused on every run, and adopting the seller's spelling instead would fork your own chain each time it changes. **A link and its canonical name are the same zone** — `Japan` and `Asia/Tokyo`, `US/Eastern` and `America/New_York` — and do not suppress; the seller's spelling is what goes on the wire. |
| `local_budget_exhausted` | Your own `ledgerLimits` ran out mid-read, the revision exceeded the SDK's size ceiling, or a row was too *wide* for the SDK to walk. | Raise `maxRevisionRows`, `maxPages` or `maxLoadMs`. `maxRevisionBytes` may only *tighten* the SDK's 256 MiB size ceiling — a larger value is **refused** with `INVALID_LEDGER_LIMITS`, because that ceiling bounds the SDK process's memory — and the breadth ceiling (262,144 containers per row) is not tunable at all. Never reported as a seller failure: a row nested deeper than the reader walks (64 levels) is `unreadable` / `reader_incompatible` instead, because no conformant tabular row has that shape. |
