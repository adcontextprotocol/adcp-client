...
npx @adcp/sdk@adcp-3.1 storyboard run --local-agent ./agent.mjs

# Single storyboard, CI-friendly JUnit output
npx @adcp/sdk@adcp-3.1 storyboard run --local-agent ./agent.mjs capability_discovery \
...
```bash
npx @adcp/sdk@adcp-3.1 storyboard run --local-agent ./agent.mjs --json | jq '.results[] | select(.overall_passed | not)'
```
