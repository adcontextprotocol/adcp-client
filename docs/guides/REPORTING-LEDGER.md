...
`createReportingManagedDeliveryRuntime` has a strict startup precondition on `automatedRecoveryWindowSeconds`, and it is worth reading before a second tenant arrives. The factory requires at least one installed managed binding, and requires the advertised value to equal **every** installed binding's Core `schedule.recoveryWindowMilliseconds`, expressed in whole seconds — the check is deployment-wide, not per account or per offering. Three consequences follow, all currently by design: a deployment whose tenants run different recovery windows has no value that satisfies the check, so the factory cannot start; a configuration installed with a sub-second `recoveryWindowMilliseconds` (Core permits any positive integer) makes the factory throw permanently, and configuration generations are immutable; and a fresh deployment with no binding yet, or one that has just offboarded its last managed tenant, fails to start rather than serving Core-only. Run one runtime per recovery-window cohort, and keep `recoveryWindowMilliseconds` on whole seconds for any configuration you intend to bind.
