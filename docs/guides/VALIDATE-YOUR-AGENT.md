...
- **Validation drops invalid fixtures.** Entries missing the dedup id (`creative_id`, `media_buy_id`, `account_id`, `account.account_id`, `format_id.{agent_url,id}`, `list_id` for property + collection lists, `standards_id` for content standards) are warn-and-dropped, not thrown — a broken test fixture shouldn't tank the request under test.
