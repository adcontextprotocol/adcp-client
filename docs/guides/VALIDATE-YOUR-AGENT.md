...
| `Skipped (not_applicable / session_probe_ungradable)` | Either the agent advertises no read-shaped (`list_*` / `get_*`) auth-required tool for the probe to call — public-tier tools (`get_adcp_capabilities`, `get_products`, `list_creative_formats`) and mutating tools are excluded by design — or the step is a positive static-credential probe the session probe cannot grade. **A static-credential-only agent with no allowlisted read tool cannot be certified**: the positive probe asserts an AdCP response body no protocol operation produces, so its branch's contribution gate stays closed. Two remedies: advertise one allowlisted read tool (`list_creatives`, `get_media_buy_delivery`, `get_signals`, `list_accounts`, …) — the durable fix — or serve RFC 9728 metadata and run with `--oauth` so the OAuth branch is verified instead. The `Skipped:` line below it carries the full reason and remedy |
