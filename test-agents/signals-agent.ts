...
const SIGNALS: Signal[] = [
  {
    ...
    deployments: [],
    pricing_options: [
      { pricing_option_id: 'po_cpm_auto', model: 'cpm', cpm: 3.50, currency: 'USD' },
    ],
  ...
  },
  {
    ...
    deployments: [],
    pricing_options: [
      { pricing_option_id: 'po_cpm_luxury', model: 'cpm', cpm: 5.00, currency: 'USD' },
    ],
  ...
  },
  {
    ...
    deployments: [],
    pricing_options: [
      { pricing_option_id: 'po_cpm_fitness', model: 'cpm', cpm: 2.00, currency: 'USD' },
    ],
  ...
  },
  {
    ...
    deployments: [],
    pricing_options: [
      { pricing_option_id: 'po_cpm_travel', model: 'cpm', cpm: 2.75, currency: 'USD' },
    ],
  ...
  signals: {
    getSignals: async (params, ctx) => {
      ...
      if (params.signal_ids) {
        results = results.filter(s =>
          params.signal_ids!.some((id: any) => id.id === s.signal_id!.id)
        );
      }
      ...
      if (params.filters?.min_coverage_percentage) {
        results = results.filter(s =>
          s.coverage_percentage >= params.filters!.min_coverage_percentage!
        );
      }
