...
function generateLlmsTxt(
  ...
  ln();
  ln(
    `**Four reference \`AccountStore\` shapes.** Pick the one whose onboarding model matches yours. **Shape A — \`InMemoryImplicitAccountStore\`**: \`resolution: 'implicit'\`, buyer-driven \`sync_accounts\` populates the auth-principal → accounts map. **Shape B — \`createOAuthPassthroughResolver\`**: \`resolution: 'explicit'\`, returns just the \`resolve\` function for adapters fronting an upstream OAuth listing endpoint (Snap, Meta, TikTok, LinkedIn — \`extract bearer → GET /me/adaccounts → match by id\`). **Shape C — \`createRosterAccountStore\`**: \`resolution: 'explicit'\`, returns a complete \`AccountStore\` for adopters who own the roster (storefront table, admin-UI-managed JSON). Supports \`resolveWithoutRef\` for tools that send no \`account\` field on the wire (\`list_creative_formats\`, \`preview_creative\`, \`provide_performance_feedback\`) — set it to return a synthetic publisher-wide entry instead of \`null\`. **Shape D — \`createDerivedAccountStore\`**: \`resolution: 'derived'\`, agents where there is no \`account_id\` on the wire and the auth principal alone routes each request (audiostack, flashtalking, single-namespace retail-media). Provide \`toAccount(ctx)\`; the factory throws \`AUTH_REQUIRED\` on missing-credential calls. Optional \`list?\` callback wires \`list_accounts\` for spec compliance and auth-scoped discovery — but \`'derived'\`-mode refusal of inline \`account_id\` still fires even when \`list?\` is set; for \`account_id\` addressing after listing, use \`createTenantStore\`. See \`docs/guides/account-resolution.md\` § \`'derived'\` mode. All four shapes live at \`@adcp/sdk/server\`.`
  );
  ln();
  ln(
