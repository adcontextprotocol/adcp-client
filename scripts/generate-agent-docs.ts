...
function generateLlmsTxt(
  ...
  ln(
    `In-process HITL callbacks can write \`return taskCtx.reject(result, reason)\` for a business decline with an artifact but no structured execution error. Push-enabled tasks settled by another process must return \`ctx.handoffToTask(producer, { settlement: 'external' })\`; its external producer context intentionally has no \`reject()\`, because it must durably queue the complete scoped handle and encrypted route before returning. Workers use \`createPostgresTaskSettlementCoordinator()\` with \`completeScopedPushTask()\`, \`failScopedPushTask()\`, or \`rejectScopedPushTask()\`; the task mutation and encrypted recovery outbox checkpoint commit together. Acknowledge work only for \`applied\` or compatible \`already_terminal\`; retry or dead-letter scope misses and conflicts.`
  );
  ln(
